...
def execute_combined_code(image_path, is_first_code=True):
    # 좌표 설정
    ...
    coordinates = coordinates_first_code if is_first_code else coordinates_second_code
    ...
    if is_b1_destination:
        ...
        stairs_and_elevator_nodes_first_code = [14, 1, 3, 15]
        stairs_and_elevator_nodes_second_code = [33, 1, 25, 3, 2, 22]

        closest_stairs_or_elevator = min(stairs_and_elevator_nodes_first_code if is_first_code else stairs_and_elevator_nodes_second_code, key=lambda x: nx.shortest_path_length(G, source=start_node, target=x, weight='weight'))
    ...
    # 레이블 표시
    plt.legend()
    print(f"Executing the {'first' if is_first_code else 'second'} code...")

    # Show the plot
    plt.show()

# 실행 예시
image_path_first_code = '디지털관 1층.jpg'
image_path_second_code = '디지털관 지하 1층.jpg'
...
while True: 
    ...
    if user_input.lower() == 'quit':
        ...
        shortest_path_result = execute_combined_code(image_path_first_code, is_first_code=True)
        print("Shortest Path Result:", shortest_path_result)
    elif user_input == '2':
        shortest_path_result = execute_combined_code(image_path_second_code, is_first_code=False)
        print("Shortest Path Result:", shortest_path_result)
    else:
        print("Invalid input. P2lease enter '1', '2', or 'quit'.")
